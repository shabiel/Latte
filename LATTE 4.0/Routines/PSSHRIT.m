PSSHRIT ;WOIFO/SG,PO - Transmits a "ping" to determine if FDB server is down and record the down time ;2017-09-19  4:07 PM
 ;;1.0;PHARMACY DATA MANAGEMENT;**136,168,164,173,180,184,C0D/LATTE,LOCAL**;9/30/97
 ;
 ;External reference to IN^PSSHRQ2 supported by DBIA 5369
 ;External reference to File 18.12 supported by DBIA 5891
 ; VEN/SMH - *C0D/LATTE - Fixes to incorrect VUID from FOIA in CUSTOM
 ;         - *C0D/LATTE - Cipro vs Warfarin will come as significant from RxNorm
 ;
 Q
...
INTERACT() ; check drug-drug interaction.
 ...
 I INFO="" S INFO=$G(^TMP($J,BASE,"OUT","DRUGDRUG","S",PSDRUG1,PSORDER,1,"PMON",9,0)) ; new line VEN/SMH for Latte
