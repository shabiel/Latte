...
EN(RESULT,DOCHAND) ; Public; Main Latte XML parser routine
 ...
 I $$VALUE^MXMLDOM(DOCHAND,HDRNODE,"pingOnly")="true" D SENDPING^C0DWRT(.RESULT) QUIT
 ;
 ...
 N INTERACTIONS,DUPCLASS ; Return arrays to RESPOND^C0DWRT
 ;
 ...
 . I SERVICE="ANSM" D
 .. D INTERACT^C0DIXN(.INTERACTIONS,.DUPCLASS,.DRUGS,PROS,.DRUGSNOTCHECKED):(DRUGDRUGCHECK!DRUGTHERAPYCHECK)
 . I SERVICE="DIT",$$SACTIVE("DIT DI SERVICE") D  ; DIT Interface available
 .. D INTERACT^C0DDIT(.INTERACTIONS,.DUPCLASS,.DRUGS,PROS,.DRUGSNOTCHECKED):(DRUGDRUGCHECK!DRUGTHERAPYCHECK)
 . I SERVICE="RXNORM",$$SACTIVE("RXNORM DI SERVICE") D  ; RxNorm Interface available
 .. D INTERACT^C0DRXN(.INTERACTIONS,.DUPCLASS,.DRUGS,PROS,.DRUGSNOTCHECKED):(DRUGDRUGCHECK!DRUGTHERAPYCHECK)
 ;
