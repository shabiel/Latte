PSSFDBRT ;WOIFO/Parviz Ostovari - Sends XML Request to PEPS via HWSC ;2015-06-11  11:20 AM
 ;;1.0;PHARMACY DATA MANAGEMENT;**136,160,11310000**;9/30/97
 ;
 ...
 ;
 ; VEN/SMH - Patch 11310000: if Latte is enabled, don't even run this.
 ; Pretend we failed, which is good enouch for our caller 
 ; (IV+11^PSIVOCDS)
 ;
 Q
GROUTE(PSSIEN,PSSOUT) ; get the routes for given drug ien in drug file from PESPS via HWSC
 ...
 ;
 ; *11310000 - Tell VISTA that this is an FDB route (-1 says ignore and pretend)
 IF $$GET^XPAR("SYS^PKG","PSS KBAN LATTE ENABLE?") D  QUIT
 . S PSSOUT(0)="-1^Latte is enabled. This functionality isn't available"
 ;
